...
def merge_contract_and_backlog_creation():
    ...
    contract_file = askopenfilename(title="Select the Contract File for Creation", filetypes=[("Excel files", "*.xlsx")])
    if not contract_file:
        print("No contract file selected.")
        return

    # Prompt the user to select the backlog file
    backlog_file = askopenfilename(title="Select the Backlog File for Creation", filetypes=[("Excel files", "*.xlsx")])
    if not backlog_file:
        print("No backlog file selected.")
        return

    # Load the specific sheet 'Dupes Removed' from the contract file
    contract_df = pd.read_excel(contract_file, sheet_name='Active Supplier Contracts')
    ...
    # Ensure IPN and Backlog CPN are strings and case-insensitive
    if 'IPN' in contract_df.columns:
        contract_df['IPN'] = contract_df['IPN'].astype(str).str.lower()
    else:
        print("Column 'IPN' not found in the contract file.")
        return
    ...
    for i, row in backlog_df.iterrows():
        match = contract_df[contract_df['IPN'] == row['Backlog CPN']]
        if not match.empty:
            backlog_df.at[i, 'Contract Price'] = match.iloc[0]['Price']  # Use the correct column name here
